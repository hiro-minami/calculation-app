...
export class InputHelper {
    ...
    private static isOperand(char: string): boolean {
        ...
        return priority.get(char) === undefined && char !== ".";
    }
}
...
export class OperatorHelper {
    ...
    private static trimZeroHelper(str: string, length: number): string {
        if (length <= 0 || (str.charAt(length) !== "0" && str.charAt(length) !== ".")) return str;
    ...
    public static operate(operandStack: number[], operatorStack: string[], operate: string = "noOperator"): string {
        if (operatorStack.length === 0) return this.optimizeNumber(operandStack[0].toFixed(5));
        let ans = 0;
        while (operatorStack.length !== 0 && priority.get(operatorStack[operatorStack.length - 1])! >= priority.get(operate)!) {
        ...
        return Number.isInteger(ans) ? ans.toString() : this.trimZero(ans.toFixed(5));
    }
}
...
export const expressionParser = (expression: string): string => {
    ...
    while (i < expression.length) {
        ...
        if (priority.get(expression.charAt(i)) === undefined) {
            let ope = "";
            let num = 0;
            while (priority.get(expression.charAt(i + num)) === undefined && i + num < expression.length) {
            ...
            } else if (priority.get(operatorStack[operatorStack.length - 1])! < priority.get(expression.charAt(i))!) {
                // 演算子スタックが空ではなく、その文字の優先順位が演算子スタックの頂上の優先順位よりも大きい場合、その文字を演算子スタックにプッシュ
                operatorStack.push(expression.charAt(i));
            } else {
                // 計算する
                OperatorHelper.operate(operandStack, operatorStack, expression.charAt(i));
    ...
    return operatorStack[0] !== null ? OperatorHelper.operate(operandStack, operatorStack) : OperatorHelper.optimizeNumber(operandStack[0].toFixed(5));
};
